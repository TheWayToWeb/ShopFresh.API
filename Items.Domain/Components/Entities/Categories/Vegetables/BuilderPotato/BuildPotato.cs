...
{
    public class BuildPotato : IBuilderPotato
    {
        ...
        public BuildPotato(Guid personId, int countInPackage, int createdDate, float weight)
        {
            PersonId = personId;
        ...
        public void SetIsWash(bool value)
        {
            if (!value) return;
            IsWash = value;
        }
        public void AddSaleWeights(List<SalesRange> value)
        {
            if (value.Count == 0) return;
            SaleWeights = value;
        }

        public SelfPotato CreatePotato()
        {
            return new SelfPotato(this);
        }
    }
}
