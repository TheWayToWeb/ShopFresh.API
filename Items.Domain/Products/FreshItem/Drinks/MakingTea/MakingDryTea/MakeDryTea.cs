...
{
    public class MakeDryTea : MakerDryTea
    {
        ...
        public int _sashetCount { get; private set; } = 0;

        public MakeDryTea() { 
        }

        public MakeDryTea(Guid personId, string itemName, int price, string imagePath) {
            PersonId = personId;
            ItemId = Guid.NewGuid();
            _itemName = itemName;
            _price = price;
        ...
        public void SashetCount(int value) => _sashetCount = value;

        public override DryTea CreateDryTea()
        {
            return new DryTea(PersonId, ItemId, _itemName, _price, _imagePath);
        }
    }
}
